...
def schedule(day, list_type1, list_type2, list_type3):
    ...
    mycursor.execute("CREATE TABLE TASK_SCHEDULE (id INT(11) NOT NULL AUTO_INCREMENT, name VARCHAR(45) NOT NULL, timestamp DATE , shift VARCHAR(45) NOT NULL, PRIMARY KEY(id)) ENGINE=InnoDB AUTO_INCREMENT=397 DEFAULT CHARSET=latin1")
    ...
    while i < int(day):
        ...
        # set ngay
        today = dt.date.today()
        sheet.cell(row= i + 6, column = 1).value = str(today + dt.timedelta(days = (i + 1)))
        ...
        for j in range (3):
            ...
            # ghi vao mysql
            mycursor.execute("INSERT INTO TASK_SCHEDULE(name, timestamp, shift) VALUES('"+ list_type1[a] +"', '"+ str(today + dt.timedelta(days = (i + 1)) )+"' ,'"+ str((j + 1))+"')")
        ...
        if (b == 3):
            ...
            b = b + 1
        # ghi vao mysql
        mycursor.execute("INSERT INTO TASK_SCHEDULE(name, timestamp, shift) VALUES('"+ list_type2[b] +"', '"+ str(today + dt.timedelta(days = (i + 1)))+"','1 2 3')")
        mycursor.execute("INSERT INTO TASK_SCHEDULE(name, timestamp, shift) VALUES('"+ list_type3[0] +"', '"+ str(today + dt.timedelta(days = (i + 1)))+"','1 2 3')")
        #print("Day " + str(i + 1) + ": " + line)
        # ghi vao excel (line by line)
        array = line.split(' ')
